    ...
    for attempt in range(max_attempts):
        try:
            ...
            if attempt < max_attempts - 1:
                print(f"   ⚠️ Intento {attempt + 1} fallido, reintentando en 2 segundos...")
                time.sleep(2)
            else:
...
def run_command(command, cwd=None, shell=False, timeout=300):
    ...
    try:
        ...
        if os.name == 'nt' and isinstance(command, list) and 'python' in command[0].lower():
            shell = True
        ...
        while True:
            if time.time() - start_time > timeout:
                process.kill()
                return False, '\n'.join(stdout_lines), f'Tiempo de espera agotado ({timeout} segundos)'
...
def create_virtual_environment(venv_dir):
    ...
    try:
        ...
        if not success:
            ...
            install_success, _, install_stderr = run_command(install_cmd, shell=os.name == 'nt')
...
def main():
    ...
    if os.name == 'nt' and not check_admin_privileges():
        ...
        print("💡 Si encuentras problemas de permisos, ejecuta PowerShell como Administrador.")
        print()
