...
const Redirect = () => {
  ...
  const validateToRedirect = async () => {
    ...
    setTimeout(() => {
      ...
      if (!b64Data) {
        setLoadingText("Hubo un problema al empaquetar la información");
  ...
  useEffect(() => {
    ...
    if (authData.user?.expires_in && sessionStorage.hasOwnProperty('_redirect')) {
      ...
      if (code) {
        ...
        setLoadingText("No se encontró un código de redirección")
        setTimeout(() => navigate('/auth/login'), 1000);
      }
    }
  }, []);
