...
const Header = () => {
    ...
    useEffect(() => {
        function handleClickOutside(e) {
            if (
                mobileMenuRef && !mobileMenuRef.current.contains(e.target) && hamburgerRef.current && !hamburgerRef.current.contains(e.target)
            ) {
                setIsOpen(false)
            }
        }
    ...
    return (
        ...
        <>
            <div id="FAKE_HEADER" className="top-0 p-8 w-full"></div> {/* FAKE_HEADER Ayuda a mantener el contenido de las paginas fuera del header (al ser este 'fixed') */}
            ...
            <header className="fixed top-0 z-50 bg-[var(--color-primario)] text-white p-4 md:px-20 w-full">
                ...
                ${isOpen ? 'max-h-96 opacity-100 mt-4' : 'max-h-0 opacity-0 mt-0 pointer-events-none'}`}
                    >
                        <li><Link to="/adminpanel" onClick={() => setIsOpen(false)}>Panel de Administrador</Link></li>
                        <li><Link to="/profile" onClick={() => setIsOpen(false)} >Perfil</Link></li>
                        <li><Link to="/logout" onClick={() => setIsOpen(false)}>Cerrar Sesión</Link></li>
                    </ul>
