...
class PropuestaEducativaService:
    ...
    def get_all():
        ...
        if not items:
            return make_response(ResponseStatus.SUCCESS, "No hay propuestas educativas disponibles", [])
        return make_response(ResponseStatus.SUCCESS, "Lista de propuestas educativas", [i.to_dict() for i in items])

    @staticmethod
    def get_by_id(id):
        item = PropuestaEducativa.query.get(id)
        if not item:
            return make_response(ResponseStatus.FAIL, "Propuesta educativa no encontrada", {"id": id})
        return make_response(ResponseStatus.SUCCESS, "Propuesta educativa encontrada", item.to_dict())
    
    @staticmethod
    def get_activas():
        items = PropuestaEducativa.query.filter_by(id_estado=1).all()
        if not items:
            return make_response(ResponseStatus.SUCCESS, "No hay propuestas activas", [])
        return make_response(ResponseStatus.SUCCESS, "Lista de propuestas activas", [i.to_dict() for i in items])

    @staticmethod
    def get_inactivas():
        items = PropuestaEducativa.query.filter_by(id_estado=2).all()
        if not items:
            return make_response(ResponseStatus.SUCCESS, "No hay propuestas inactivas", [])
        return make_response(ResponseStatus.SUCCESS, "Lista de propuestas inactivas", [i.to_dict() for i in items])
    ...
    def create(data):
        ...
        try:
            ...
            db.session.commit()
            return make_response(ResponseStatus.SUCCESS, "Propuesta educativa creada correctamente", {"id": nueva.id})
        except Exception as e:
            db.session.rollback()
            return make_response(ResponseStatus.ERROR, "Error al crear propuesta educativa", {"error": str(e)})
    ...
    def update(id, data):
        ...
        try:
            ...
            db.session.commit()
            return make_response(ResponseStatus.SUCCESS, "Propuesta educativa actualizada", item.to_dict())
        except Exception as e:
            db.session.rollback()
            return make_response(ResponseStatus.ERROR, "Error al actualizar la propuesta educativa", {"error": str(e)})
    ...
    def delete(id):
        ...
        if item.id_estado == 2:
            return make_response(ResponseStatus.FAIL, "La propuesta ya está inactiva", {"estado": "inactiva"})

        try:
            item.id_estado = 2
            db.session.commit()
            return make_response(ResponseStatus.SUCCESS, "Propuesta educativa inactivada", {"id": id})
        except Exception as e:
            db.session.rollback()
            return make_response(ResponseStatus.ERROR, "Error al inactivar propuesta", {"error": str(e)})