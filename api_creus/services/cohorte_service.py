...
class CohorteService:
    ...
    def get_by_estado(estado_id):
        ...
        return (make_response(ResponseStatus.SUCCESS, mensaje, [i.to_dict() for i in items]), 200,)

    @staticmethod
    def get_by_id(id):
        item = Cohorte.query.get(id)
        if not item:
            return (make_response(ResponseStatus.FAIL, "Cohorte no encontrada", {"id": id}), 404,)
        return (make_response(ResponseStatus.SUCCESS, "Cohorte encontrada", item.to_dict()), 200,)
    ...
    def create(data):
        ...
        try:
            ...
            db.session.commit()
            return (make_response(ResponseStatus.SUCCESS, "Cohorte creada correctamente", {"id": nueva.id}), 200,)
        except Exception as e:
            db.session.rollback()
            return (make_response(ResponseStatus.ERROR, "Error al crear cohorte", {"error": str(e)}), 500,)
    ...
    def update(id, data):
        ...
        try:
            ...
            db.session.commit()
            return (make_response(ResponseStatus.SUCCESS, "Cohorte actualizada correctamente", item.to_dict()), 200,)
        except Exception as e:
            db.session.rollback()
            return (make_response(ResponseStatus.ERROR, "Error al actualizar cohorte", {"error": str(e)}), 500,)
    ...
    def delete(id):
        ...
        try:
            ...
            db.session.commit()
            return (make_response(ResponseStatus.SUCCESS, "Cohorte inactivada correctamente", {"id": id}), 200,)
        except Exception as e:
            db.session.rollback()
            return (make_response(ResponseStatus.ERROR, "Error al eliminar cohorte", {"error": str(e)}), 500,)
