...
#modelo del json que hay que borrar despues de actualizar todos los routes y schemas⬇️
from flask import jsonify

def success_object(data, message="Recurso obtenido correctamente", status_code=200):
    return jsonify({
        "status": "success",
        "data": data,
        "message": message
...
def success_list(data, message="Lista obtenida", status_code=200):
    return jsonify({
        "status": "success",
        "data": data,
        "total": len(data),
...
def success_empty(message="No se encontraron resultados", status_code=200):
    return jsonify({
        "status": "success",
        "data": [],
        "total": 0,
...
def success_created(resource_id, message="Recurso creado correctamente", status_code=201):
    return jsonify({
        "status": "success",
        "message": message,
        "data": {
...
def error_response(message="Error de validación", errors=None, status_code=400):
    return jsonify({
        "status": "error",
        "message": message,
        "errors": errors or {}
